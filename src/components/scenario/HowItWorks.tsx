...
const PARAMS: ParamRow[] = [
  ...
  { sym: "\\tau", meaning: "Deployment → revenue lag", value: "6 mo" },
    ...
    <div className="exp-howto">
      ...
      <section className="exp-howto-sec">
        ...
        <div className="exp-howto-eq">
          ...
          <p className="exp-howto-cap">
            User dynamics: acquisition (external marketing + word-of-mouth) minus losses (churn +
            competition), with a demand-noise term so each run is a scenario, not a single prophecy.
          </p>
        </div>
        ...
        <div className="exp-howto-eq">
          ...
          <p className="exp-howto-cap">
            Quality maps: it lowers churn through a threshold &ldquo;cliff&rdquo; and raises per-user
            margin linearly. These are the only two places quality enters.
          </p>
        </div>
        ...
        <div className="exp-howto-eq">
          <Tex block>
            {
              "\\Pi(t)=\\Theta(t-\\tau)\\big[m(Q)-\\Delta m_{\\text{shock}}\\,\\Theta(t-t_{\\text{shock}})\\big]N - c_{\\mathrm{ac}}\\big[\\chi(Q)+\\varphi\\tfrac{t}{T}\\big]N - F"
            }
          </Tex>
          <p className="exp-howto-cap">
            ...
            users and a fixed overhead. Cumulative profit is the time-integral of <Tex>{"\\Pi(t)"}</Tex>{" "}
            over the horizon.
          </p>
        </div>
        ...
        <p className="exp-prose exp-howto-note">
          ...
          margin and net profit — the separate cost line (CAC + fixed) stays smooth through the shock.
        </p>
      </section>
      ...
      <section className="exp-howto-sec">
        ...
        <p className="exp-prose">
          ...
          the two the demo varies — <Tex>{"Q"}</Tex> (the quality choice) and <Tex>{"Q^{\\ast}"}</Tex>{" "}
          (where the market sets the bar). The <strong>calibration</strong> worth grounding in real
          data is the competition rate <Tex>{"\\varphi"}</Tex>, the acquisition cost{" "}
          <Tex>{"c_{\\mathrm{ac}}"}</Tex>, the fixed cost <Tex>{"F"}</Tex>, the churn-cliff shape and
          the margin slope <Tex>{"\\Delta m"}</Tex>. Everything else only scales magnitudes without
          changing the story.
        </p>
      </section>
