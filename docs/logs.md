...
**THE STORY (fixed 2026-07-02, round 10 — this is the narrative, do not drift from it) `[fact]`, dm 6, Q* 0.5, baseline vec; cumProfit €M Lean/Balanced/Premium (demo scale, all <10):**
- **Status quo** 2.29 / 4.45 / **6.27** → all three tiers are viable businesses; Premium earns the most, Lean leaves money on the table but is safe.
- **Pricing shock ×3 @ m16** **1.03** / 0.83 / −0.04 → **Lean wins a price war** (cheapest serving = insurance); Premium's frontier serving is its exposed flank. Response: "Shift serving off-vendor now" +€1.4M for Balanced (from m16).
- **Regulatory pressure** 0.67 / 2.83 / **4.70** → compresses everyone, hits Lean's thin margin hardest relatively; response = "Build your own compliance muscle" (build absorbs 30% of compliance).
- **Open-source adoption** −3.64 / −2.38 / **−0.17** → the −0.2 quality hit shoves every tier toward its bar; Premium's headroom absorbs it best, Lean crashes. Response: "Build it, then charge for it" +€1.3M for Balanced.
- Flat-price crossovers: Premium leads to ~×2, **Balanced leads ×2–×2.5**, **Lean wins past ×2.5**. Aggression optima: Lean ≈20, Balanced/Premium ≈40, everyone cliffs by 60.
The mechanism that makes tiers comparable: **tier-fair quality bar** `bar = Q*·Q/0.6` (a Lean product is judged against its own modest promise, not Premium's). Mitigations move the four levers ONLY (never the tier) and are simulated as responses from the realization month; the mitigation chart/table starts AT that month.
...
### 2026-07-02 (round 9 — copy style pass: no em dashes, LaTeX for Greek, Problem page rewrite)
User directive: no em dashes anywhere (use ; or :), Greek/math symbols always via LaTeX (no raw Unicode in prose), and the Problem page copy was incoherent.
- Em dashes removed from ALL rendered copy (sed + hand pass over every file; broken sed parentheticals — a dash pair becoming two semicolons mid-clause — repaired by hand in HowItWorks, Mitigation tag, index.tsx, model.ts strings). Verified 0 em dashes in the rendered page body (Playwright innerText check). Also cleaned comments/styles.css/README.
- Greek via KaTeX in prose: evaluator scaling note (Δm, Q*), Mitigation card chips (Q*, Δm via Tex; required scoping `.exp-mit-card-vec > span` so chip styling stops leaking into KaTeX's internal spans), chart guide "τ revenue" → "revenue lag" (SVG labels can't KaTeX). KEPT: CausalDiagram node glyphs (χ, ϕ, Δm, Π, Q∗) as native SVG text in the KaTeX_Math font — real KaTeX in SVG needs foreignObject (mobile-unsafe, prior design decision); they already render in the LaTeX font. AiInsight prompt strings are LLM input, not UI — left plain.
- Problem page rewritten (ProblemFrame.tsx): THE DECISION (company must decide, four choices lock each other in), THE CORE INSIGHT (two paragraphs: why margin-per-user hides the decision → four interacting quantities; what the tool does: simulate + P&L, profit is the output not an input), WHAT MAKES THIS HARD (plain statement, dropped "Considering our use case"). Journey blurb: "What you are deciding, and why a single margin-per-user number cannot decide it."
- Verified: tsc 0, build 0, Playwright screenshots (problem, causal, mitigation chips).

### 2026-07-02 (round 10 — ONE CONSISTENT STORY: tier-fair bar, ÷100 scale, response-view mitigation, simpler formulas)
User (frustrated, rightly): hedge jargon unexplained; mitigation chart still showed pre-shock history; "Build + move upmarket" tier-jump under OSS implied an unbelievable swing; Lean/Balanced never viable (Premium always won); numbers felt inflated (wanted cum <€10M, ÷100); the four risk factors read as one blob; too many hidden nonlinear coefficients (0.4·Δm/12 serve bump; 0.30(0.6v+0.4b) buffer); s node over-explained; Π delta overflowed; mitigation names not suggestive.
**Model (all verified by harness):**
- **Tier-fair quality bar** `tierBar(Q*,Q)=Q*·Q/0.6` threaded through simulate/causal/risk-cliff/switched (`qRef 0.6 [design]`). This is THE fix for "Premium always wins": one shared bar executed Lean at Premium's standard in every scenario. Story table now differentiates (see RESUME POINT). References gets a form-row for it.
- **÷100 demo scale** (K 20k–150k, N0 400, F €40k/50k/15k/30k /mo, profitDisplay 10) per explicit user direction; supersedes the morning's ×10 flagship restore (references.md keeps the flagship anchors ×100 for real-world quoting). Display units: monthly charts €k, users in k, F in €k/mo, cum in €M 1-decimal.
- **Simplifications** (user: "too many hidden nonlinear effects"): `scalingServeBump` REMOVED → s(t)=s₀·f(Q)·ρ(t), three explainable factors (aggression is punished by the churn cliff, which carries the story alone); compliance buffer simplified to `1−0.30·b` (build-only, ONE coefficient; radar regulatory axis + "absorb" mitigation aligned). Story survived both: crossovers moved to ×2 (P→B) and ×2.5 (B→L); Recommendation flip text updated to the recomputed values.
- **Mitigations are levers-only** (no strat±1 candidates; guardrail keeps the tier): "switch strategy" is not a mitigation of a strategy. Labels rewritten as actions ("Shift serving off-vendor now", "Promise less, keep your users", "Build your own compliance muscle", "Rebuild the lost quality yourself", "Re-centre all levers"...). Floor rescaled to −max(€0.05M, 2%).
- **Mitigation view starts at the realization month**: chart x-axis and table ("Cumulative m16–54", "vs staying") sliced at switchMonth; briefing + apply-text quote from-response cumulatives; guide relabeled "shock hits, you respond". Pre-shock history is no longer drawn (it was fixed history no response can change).
**Diagram/copy:** s node plain ("serving €5.8/user" + under shock "vendor ×3.0, you pay ×2.3"; nothing at ×1); "hedge" → "off-vendor share" everywhere user-facing; Π delta rendered BELOW the diamond (no overflow); Q* node shows "your tier's bar 0.50"; K/N/F in k/€k; four-factor differentiation paragraph on the risk stage (weather / how much weather you feel / quality you own / what you promise+charge); tier-bar equation + caption in HowItWorks (Q*_tier=Q*·Q/0.6); param table updated to demo scale; AiInsight prompt updated.
**Verified `[fact]`:** tsc 0 · build 0 · harness: story table + winners, cum<10 everywhere, switched identity 0-diff, 12 mitigation cells credible with NO tier jumps, regulatory axis owned by reg & buffered by build, crossover thresholds match Recommendation copy · Playwright: causal SQ+shock, mitigation-from-m16, risk stage.
STILL PENDING: `standalone/` (user: forget for now).

### 2026-07-02 (round 11 — risk/P&L consistency + expectations framing)
- **User caught a real contradiction:** under OSS every axis sat below its tipping line while all tiers lost money. Root cause: the scaling-risk cliff term measured the LINEAR quality gap `(bar−Q_eff)/0.4`, but the churn logistic (κ=12) makes a 0.03 gap already deadly; risk and P&L used different rulers. **Fix: cliff = realized churn position** `(χ(Q_eff, bar)−χmin)/(χmax−χmin)`. Verified correspondence grid `[fact]`: SQ = all profitable, all calm; shock = cost 74 ⚠ for all (P loses); regulatory = reg 77 ⚠ (compressed, positive — a load warning, not a loss claim); OSS = scaling 88/77 ⚠ for L/B (both lose), Premium 64 calm at −0.17 breakeven. Ownership intact (dial 6→100; build cuts OSS scaling 91→60). OSS/Balanced headline mitigation is now "Promise less, deliver more" (+€2.63M from response).
- **Scaling-intensity framing fixed (user: "raising Q* without Q is illogical"):** Q* is now consistently framed as the EXPECTATIONS bar; expectations rise with price (that is WHY Δm and Q* share a dial and cannot move apart); delivered quality is a separate investment (tier, build); the gap is the modeled tipping risk. Deliberately did NOT couple Q to Q* (that would make over-promising impossible and delete the cliff/optimum story). Copy aligned: slider note, axis-map ("price premium Δm & users' expectations Q*"), diagram Q* node ("users' bar"), risk-axis rises-text, tipping explanation, HowItWorks finding + lever section, AiInsight prompt.
- Merged parallel Lovable edits (Strategy sweep tab removed by user; tier legend retitled; scrollbar-gutter fix). Story numbers for the pitch verified this round: insurance (B, SQ 4.85→4.04 vs shock −0.71→+2.37 across resil 0→100); retention beats margin (build 0→100: users +52% vs ARPU +17%, cum 2.2→6.3); price-war ladder ×2/×2.5; per-tier ambition optima 20/40/40; retreat-under-shock rejected by the model (reach 80 beats 20 even at ×3).
