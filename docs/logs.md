...
2. **Causal-diagram redesign** — proposed: the 4 sliders as explicit driver-nodes wired to the variables they change (reach→K/N, independence→ρ price, in-house build→Q_eff, scaling→Q*&Δm), each with live value + ↑/↓ vs baseline. **AWAITING USER CONFIRM on direction before building** (it's a layout change; don't guess).
3. **Task #7:** realign `standalone/` port to the new model (deferred until the React app is signed off).
(Resolved in round 6: HowItWorks copy for the new couplings; churn-form A/B → decided for Q_eff; CAC now on gross adds; tier-priced serving kills the Premium-dominance loophole.)
